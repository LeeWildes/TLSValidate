...
class TestSSLServer {
	...
	{
		Console.WriteLine(
"Usage: TLSValidate [ options ] servername [ port ]");
		Console.WriteLine(
"Options:");
		Console.WriteLine(
"  -h                print this help");
...
"  -text fname       write text report in file 'fname' ('-' = stdout)");
        Console.WriteLine(
"  -html fname       write html report in file 'fname' ('-' = stdout)");
		Console.WriteLine(
...
"  -log fname        write debug log in file 'fname' ('-' = stdout)");
	...
	static void Process(string[] args)
	{
		 ...
		 */
		if (textOut == null && jsonOut == null && htmlOut == null) {
			textOut = "-";
		}
